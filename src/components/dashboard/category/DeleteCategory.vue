...
const deleteCategory = async () => {
  ...
  try {
    const response = await axios.delete(`http://localhost:8000/api/categories/${props.categoryId}`);
...
<template>
  <v-card>
    ...
    <v-card-text>
      Warning: Deleting this category will also uncategorize all products associated with it. This action cannot be
