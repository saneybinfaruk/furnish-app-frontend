...
console.log(`subCategoryId ==> ${props.subCategoryId}, categoryId ==> ${props.categoryId} `);
...
const fetchCategories = async () => {
  ...
  selectedCategory.value = categories.value.find(c => c.id === props.categoryId).id;
...
const fetchSubCategory = async () => {
  const response = await axios.get(`http://127.0.0.1:8000/api/subcategories/${props.subCategoryId}`);
  newSubCategory.value = await response.data.name;
}
...
const addNewSubCategory = async () => {
  ...
  try {
    const response = await axios.put(`http://localhost:8000/api/subcategories/${props.subCategoryId}`, {
